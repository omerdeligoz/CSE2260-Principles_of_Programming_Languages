...
public class Lexer {
    ...
    static ArrayList<String> tokenNames = new ArrayList<>();
        ...
        while (input.hasNext()) {
            ...
            for (int i = 0; i < line.length(); i++) {
                ...
                if (isSpecialChars(ch)) {
                    switch (ch) {
                        case '(' -> {
                            tokens.add("LEFTPAR " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                        case ')' -> {
                            tokens.add("RIGHTPAR " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                        case '[' -> {
                            tokens.add("LEFTSQUAREB " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                        case ']' -> {
                            tokens.add("RIGHTSQUAREB " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                        case '{' -> {
                            tokens.add("LEFTCURLYB " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                        case '}' -> {
                            tokens.add("RIGHTCURLYB " + row + ":" + col);
                            tokenNames.add(String.valueOf(ch));
                        }
                    }
                } else {
    ...
    public static void printTokens() throws IOException {
        FileWriter output = new FileWriter("lexeroutput.txt");
